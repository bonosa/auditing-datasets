...
def bad_visibility(visibility,minimum):
    ...
    if visibility["units"] == "FT":
        min_visibility = visibility.get("minimum", visibility["prevailing"]) / 5280
    else:
        min_visibility = visibility.get("minimum", visibility["prevailing"])

    return minimum > min_visibility
...
def bad_winds(winds,maxwind,maxcross):
    ...
    if winds == "calm":
        ...
        #winds is a dictionary
        wind_speed = winds["speed"]
        #the next two are optional so we give them default values
        wind_crosswind = winds.get("crosswind", 0)
        wind_gusts = winds.get("gusts", wind_speed)
        wind_units = winds["units"]
        #convert MPS to KT
        if wind_units == "MPS":
            wind_speed *= 1.94384
            wind_crosswind *= 1.94384
            wind_gusts *= 1.94384

        return wind_speed > maxwind or wind_gusts > maxwind or wind_crosswind > maxcross
...
def bad_ceiling(ceiling,minimum):
    ...
    Precondition: minimum is a float or int
    """
    # a few clouds or scattered c;ouds are not relevant
    if ceiling == 'clear':
        return False
    ...
    for layer in ceiling:
        if layer['type'] in ['broken', 'overcast', 'indefinite ceiling']:
            height = layer['height']
            if height < min_height:
                min_height = height

    return min_height < minimum
...
def get_weather_report(takeoff,weather):
    ...
    for key in weather.keys():
        current_time = utils.str_to_time(key, takeoff.tzinfo)
        #print(f"Takeoff time: {takeoff}, Current time: {current_time}")  # Debugging statement
        if current_time < takeoff:
            difference = takeoff - current_time
            if min_difference is None or difference < min_difference:
                min_difference = difference
                closest_key = current_time

    
    return weather[closest_key.isoformat()] if closest_key else None
...
def get_weather_violation(weather,minimums):
    ...
    if weather is None:
        return "Unknown"
    visibility_violation = bad_visibility(weather["visibility"],minimums[1])
    #visibility_violation = bad_visibility(weather["visibility"] if "visibility" in weather else {"units": "FT", "prevailing": 0}, minimums[1])
    #visibility_violation = bad_visibility(weather["visibility"] if "visibility" in weather else {"units": "FT", "prevailing": 0}, float(minimums[1]))
   
    winds_violation = bad_winds(weather["wind"], minimums[2], minimums[3])
    ceiling_violation = bad_ceiling(weather["sky"], minimums[0])

    if visibility_violation and winds_violation and ceiling_violation:
        return "Weather"
    elif visibility_violation and winds_violation:
        return "Weather"
    elif visibility_violation and ceiling_violation:
        return "Weather"
    elif winds_violation and ceiling_violation:
        return "Weather"
    elif visibility_violation:
        return "Visibility"
    elif winds_violation:
        return "Winds"
    elif ceiling_violation:
        return "Ceiling"
    else:
        return ""
...
def list_weather_violations(directory):
    ...
    for lesson in lessons[1:]:
        ...
        violation = ''
        if "timezone" in daytimes:
           timezone = daytimes["timezone"]
        else:
            daytimes["timezone"] = "UTC"
        ...
        if lesson[5]=="VFR":
            vfr=True
        required=pilots.get_minimums(category,lesson[6],instructed,vfr,allday,minimums)
    ...
    return result
    #violations.list_weather_violations(tests) is missing the flight 2017-03-07T08:00:00-05:00 for pilot S00